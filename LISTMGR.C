#pragma	title("List Box Replacement  --  Version 1.0 -- (ListMgr.C)")
...
/*					  INT sRight);			*/
/*	       static LONG lSetItemText(HHEAPMEM hHeap,	HWND hWnd,	*/
...
static LONG lSetItemText(HHEAPMEM hHeap, HWND hWnd, PLISTITEM pli, PSZ pszText);
static VOID RefreshList(PLISTBOXWIN plbw, LONG iItem);
...
do
   {
   while ( (stricmp((CHAR *)pli[i].pszText, (CHAR *)pszText) < 0) && (i	< sRight) )
       ++i;
   while ( (stricmp((CHAR *)pli[n].pszText, (CHAR *)pszText) > 0) && (n	> sLeft) )
       --n;
...
if ( pszText )
    ...
    memcpy((VOID *)(pli->pszText = (PSZ)HeapMalloc(hHeap, (pli->cText =	strlen((CHAR *)pszText)) + 1)),
	   (VOID *)pszText, strlen((CHAR *)pszText));
...
for ( i	= 0; i < plbw->cItems; i++ )
   plbw->plc[0].ali[i].fl = ((plbw->plc[0].ali[i].fl & 0x0000ffffUL) <<	16UL) |
			    (plbw->plc[0].ali[i].fl & 0x0000ffffUL);
...
for ( i	= 0; i < plbw->cItems; i++ )
   plbw->plc[0].ali[i].fl = (plbw->plc[0].ali[i].fl >> 16UL) |
			    ((plbw->plc[0].ali[i].fl & 0x0000ffffUL) <<	16UL);
...
if ( iSelected < plbw->cItems )
   ...
   if (	fSelect	)
       plbw->plc[iCol].ali[iSelected].fl |= LI_SELECTED;
   else
       plbw->plc[iCol].ali[iSelected].fl &= ~LI_SELECTED;
...
if ( iSelected < plbw->cItems )
   ...
   if (	plbw->flStyle &	LS_MULTIPLESEL )
	   ...
	   for ( i = 0;	i < plbw->cItems; i++ )
	       if ( (plbw->plc[iCol].ali[i].fl & LI_SELECTED) &&
		    (i != iSelected) &&
		    (i != plbw->iSelected) )
		   {
		   SaveSelectState(0L, i);
		   plbw->plc[iCol].ali[i].fl &=	~LI_SELECTED;
	   ...
	   if (	plbw->iSelected	!= LIT_NONE )
	       ...
	       plbw->plc[iCol].ali[plbw->iSelected].fl &= ~(LI_SELECTED	| LI_FOCUS);
	   ...
	   plbw->plc[iCol].ali[plbw->iSelected = iSelected].fl |= LI_SELECTED;
	   }
       else
	   {
	   plbw->plc[iCol].ali[iSelected].fl &=	~(LI_SELECTED |	LI_FOCUS);
...
plbw->plc[0].ali = (PLISTITEM)HeapMalloc(plbw->plc[0].hHeap = hHeap,
					 ((cItems / 8UL) + 1UL)	* 8UL *	sizeof(LISTITEM));
...
for ( i	= 0; i < cItems; i++ )
   {
   if (	(lSetItemText(plbw->plc[0].hHeap, plbw->hWnd, &plbw->plc[0].ali[i],
		      pb) > plbw->cxItem) && (plbw->flStyle & LS_HORZSCROLL) )
       plbw->cxItem = plbw->plc[0].ali[i].cxItem;
   pb += strlen((CHAR *)pb) + 1;
   }
}
...
switch ( msg )
   ...
   case	LM_DELETEITEM :
	   ...
	   if (	--plbw->cItems == 0UL )
	       ...
	       HeapFree(plbw->plc[0].hHeap, plbw->plc[0].ali[LONGFROMMP(mp1)].pszText);
	       memmove(&plbw->plc[0].ali[LONGFROMMP(mp1)], &plbw->plc[0].ali[LONGFROMMP(mp1) + 1],
		       (UINT)((plbw->cItems - LONGFROMMP(mp1)) * sizeof(LISTITEM)));
	       }
	   ...
	   RefreshList(plbw, LONGFROMMP(mp1));
	   }
   ...
   case	LM_INSERTITEM :
	   ...
	   if (	(plbw->cItems %	8UL) ==	0 )
	       plbw->plc[0].ali	= (PLISTITEM)HeapRealloc(plbw->plc[0].hHeap, plbw->plc[0].ali,
							 (((plbw->cItems / 8UL)	+ 1UL) * 8UL) *
							 sizeof(LISTITEM));
	   }
       else
	   {
	   ...
	   plbw->plc[0].ali = (PLISTITEM)HeapMalloc(hHeap, 8UL * sizeof(LISTITEM));
	   }
	   ...
	   case	LIT_END	:
	       ...
	       if ( (lSetItemText(plbw->plc[0].hHeap, hWnd, &plbw->plc[0].ali[plbw->cItems++],
				  PVOIDFROMMP(mp2)) > plbw->cxItem) && (plbw->flStyle &	LS_HORZSCROLL) )
		   plbw->cxItem	= plbw->plc[0].ali[plbw->cItems	- 1].cxItem;
	   ...
	   case	LIT_SORTDESCENDING :
	       ...
	       if ( (lSetItemText(plbw->plc[0].hHeap, hWnd, &plbw->plc[0].ali[plbw->cItems],
				  PVOIDFROMMP(mp2)) > plbw->cxItem) && (plbw->flStyle &	LS_HORZSCROLL) )
		   plbw->cxItem	= plbw->plc[0].ali[plbw->cItems].cxItem;
	       pli = &plbw->plc[0].ali[plbw->cItems];
		   ...
		   SortDescending(plbw->plc[0].ali, 0, (INT)(pli->iIndex = (ULONG)plbw->cItems++));
	       else
		       /* Sort the list	in ascending order		*/

		   SortAscending(plbw->plc[0].ali, 0, (INT)(pli->iIndex	= (ULONG)plbw->cItems++));
	       RefreshList(plbw, (INT)pli->iIndex);
	       if ( plbw->fFocus && (plbw->iFocus == LIT_NONE) )
		   SetFocus(plbw, 0L);
	   ...
	   default :
	       ...
	       if ( ((LONG)LONGFROMMP(mp1) >= 0L) && ((LONG)LONGFROMMP(mp1) <= plbw->cItems) )
		   {
		   ...
		   if (	LONGFROMMP(mp1)	!= plbw->cItems	)
			   ...
			   plbw->plc[0].ali[i] = plbw->plc[0].ali[i - 1];
		   else
		   ...
		   if (	(lSetItemText(plbw->plc[0].hHeap, hWnd,	&plbw->plc[0].ali[LONGFROMMP(mp1)],
				      PVOIDFROMMP(mp2))	> plbw->cxItem)	&& (plbw->flStyle & LS_HORZSCROLL) )
		       plbw->cxItem = plbw->plc[0].ali[LONGFROMMP(mp1)].cxItem;
   ...
   case	LM_QUERYITEMHANDLE :
       ...
       if ( plbw->cItems && (LONGFROMMP(mp1) < plbw->cItems) )
	   return(MRFROMLONG(plbw->plc[0].ali[LONGFROMMP(mp1)].ulHandle));
      break;
   ...
   case	LM_QUERYITEMTEXT :
	   ...
	   {
	   memcpy(PVOIDFROMMP(mp2), plbw->plc[0].ali[SHORT1FROMMP(mp1)].pszText,
		  (plbw->plc[0].ali[SHORT1FROMMP(mp1)].cText + 1) < SHORT2FROMMP(mp1) ?
		  (plbw->plc[0].ali[SHORT1FROMMP(mp1)].cText + 1) :
		  SHORT2FROMMP(mp1));
	   return(MRFROMLONG(plbw->plc[0].ali[SHORT1FROMMP(mp1)].cText < SHORT2FROMMP(mp1) ?
			     plbw->plc[0].ali[SHORT1FROMMP(mp1)].cText :
			     SHORT2FROMMP(mp1)));
	   }
       else
	   return(0L);
   ...
   case	LM_QUERYITEMTEXTLENGTH :
       ...
       if ( plbw->cItems && (LONGFROMMP(mp1) < plbw->cItems) )
	   return(MRFROMLONG(plbw->plc[0].ali[LONGFROMMP(mp1)].cText));
       break;
   ...
   case	LM_QUERYSELECTION :
	   ...
	   if (	plbw->flStyle &	(LS_MULTIPLESEL	| LS_EXTENDEDSEL) )
		   ...
		   {
		   for ( i = 0;	i < plbw->cItems; i++ )
		       if ( plbw->plc[0].ali[i].fl & LI_SELECTED )
			   return(MRFROMLONG(i));
   ...
   case	LM_SEARCHSTRING	:
	   ...
	   if (	(ULONG)SHORT1FROMMP(mp1) & LSS_PREFIX )
	       {
	       usLen = strlen((CHAR *)PVOIDFROMMP(mp2));

	       if ( (ULONG)SHORT1FROMMP(mp1) & LSS_CASESENSITIVE )
		   {
		   for ( i = n;	i < plbw->cItems; i++ )
		       if ( strncmp((CHAR *)plbw->plc[0].ali[i].pszText, (CHAR *)PVOIDFROMMP(mp2), usLen) == 0 )
			   return(MRFROMLONG(i));
		   ...
		   {
		   for ( i = n;	i < plbw->cItems; i++ )
		       if ( strnicmp((CHAR *)plbw->plc[0].ali[i].pszText, (CHAR	*)PVOIDFROMMP(mp2), usLen) == 0	)
			   return(MRFROMLONG(i));
		   ...
		   {
		   for ( i = n;	i < plbw->cItems; i++ )
		       if ( strstr((CHAR *)plbw->plc[0].ali[i].pszText,	(CHAR *)PVOIDFROMMP(mp2)) == 0 )
			   return(MRFROMLONG(i));
		   ...
		   if (	(ULONG)SHORT1FROMMP(mp1) & LSS_CASESENSITIVE )
		       {
		       for ( i = n; i <	plbw->cItems; i++ )
			   if (	strcmp((CHAR *)plbw->plc[0].ali[i].pszText, (CHAR *)PVOIDFROMMP(mp2)) == 0 )
			       return(MRFROMLONG(i));
		       ...
		       {
		       for ( i = n; i <	plbw->cItems; i++ )
			   if (	stricmp((CHAR *)plbw->plc[0].ali[i].pszText, (CHAR *)PVOIDFROMMP(mp2)) == 0 )
			       return(MRFROMLONG(i));
   ...
   case	LM_SELECTITEM :
	   ...
	   for ( i = 0;	i < plbw->cItems; i++ )
		   ...
		   {
		   SaveSelectState(0L, i);
		   plbw->plc[0].ali[i].fl &= ~LI_SELECTED;
	   ...
	   if (	LONGFROMMP(mp1)	< plbw->cItems )
		   ...
		   SaveSelectState(0L, LONGFROMMP(mp1));
		   if (	(BOOL)LONGFROMMP(mp2) )
		       {
		       RemoveFocus(plbw);
		       plbw->plc[0].ali[LONGFROMMP(mp1)].fl |= (LI_SELECTED | LI_FOCUS);
		       FocusChange(plbw, plbw->iFocus =	LONGFROMMP(mp1), TRUE);
		       if ( plbw->iSelected == -1 )
			   plbw->iAnchor = plbw->iSelected = plbw->iFocus;
		       }
		   else
		       {
		       plbw->plc[0].ali[LONGFROMMP(mp1)].fl &= ~LI_SELECTED;
		       for ( i = LONGFROMMP(mp1), plbw->iSelected = -1;	i >= 0;	i-- )
			   if (	plbw->plc[0].ali[i].fl & LI_SELECTED )
			       {
			       plbw->iSelected = i;
			       break;
		       ...
		       if ( plbw->iSelected == -1 )
			   for ( i = LONGFROMMP(mp1); i	< plbw->cItems;	i++ )
			       if ( plbw->plc[0].ali[i].fl & LI_SELECTED )
				   {
				   plbw->iSelected = i;
				   break;
		   ...
		   DrawSelection(plbw, LONGFROMMP(mp1));
		   }
	       else
		   {
		       /* Single selection only	allowed	for the	list	*/
		   ...
		   if (	(BOOL)LONGFROMMP(mp2) )
			   ...
			   {
			   SaveSelectState(0L, plbw->iSelected);
			   plbw->plc[0].ali[plbw->iSelected].fl	&= ~(LI_SELECTED | LI_FOCUS);
		       ...
		       SaveSelectState(0L, LONGFROMMP(mp1));
		       plbw->plc[0].ali[plbw->iFocus = plbw->iSelected = LONGFROMMP(mp1)].fl |=	(LI_SELECTED | LI_FOCUS);
		       }
		   else
		       {
		       SaveSelectState(0L, LONGFROMMP(mp1));
		   ...
		   if (	plbw->iVertScroll != iVertScroll )
			   ...
			   DrawSelection(plbw, plbw->iSelected);
		   if (	LONGFROMMP(mp1)	)
		       FocusChange(plbw, LONGFROMMP(mp1), TRUE);
		   }
   ...
   case	LM_SETITEMHANDLE :
       ...
       if ( plbw->cItems && (LONGFROMMP(mp1) < plbw->cItems) )
	   {
	   plbw->plc[0].ali[LONGFROMMP(mp1)].ulHandle =	(ULONG)LONGFROMMP(mp2);
	   return(MRFROMLONG(TRUE));
	   }
       break;
   ...
   case	LM_SETTOPINDEX :
	   ...
	   if (	LONGFROMMP(mp1)	< plbw->cItems	)
		   ...
		   if (	LONGFROMMP(mp1)	> (plbw->cItems	- plbw->cLinesPage) )
			   ...
			   plbw->iVertScroll = LONGFROMMP(mp1);
	       else
		   plbw->iVertScroll = LONGFROMMP(mp1);
	   else
	       plbw->iVertScroll = 0L;
...
{
HHEAPMEM     hHeap;		   /* Heap Handle			*/
PLISTBOXITEM albi;		   /* List Box Item Input Pointer	*/
PLISTBOXWIN  plbw;		   /* List Box Internal	Data Pointer	*/
...
switch ( msg )
   ...
   case	LMX_ADDARRAY :
	   ...
	   if (	plbw->cItems ==	0 )
	       ...
	       plbw->plc[0].ali	= (PLISTITEM)HeapMalloc(plbw->plc[0].hHeap = hHeap,
							(((LONGFROMMP(mp1) / 8UL) + 1UL) * 8UL)	*
							sizeof(LISTITEM));
	       }
	   else
	       plbw->plc[0].ali	= (PLISTITEM)HeapRealloc(plbw->plc[0].hHeap, plbw->plc[0].ali,
							 ((((LONGFROMMP(mp1) + plbw->cItems) / 8UL) + 1UL) * 8UL) *
							 sizeof(LISTITEM));
	   ...
	   for ( i = 0,	n = plbw->cItems; i < LONGFROMMP(mp1); i++, n++	)
	       if ( apsz[i] == NULL )
		   lSetItemText(plbw->plc[0].hHeap, hWnd, &plbw->plc[0].ali[n],	(PSZ)"") ;
	       else
		   if (	(lSetItemText(plbw->plc[0].hHeap, hWnd,	&plbw->plc[0].ali[n],
				      apsz[i]) > plbw->cxItem) && (plbw->flStyle & LS_HORZSCROLL) )
		       plbw->cxItem = plbw->plc[0].ali[n].cxItem;

		       /* Save the list	box length and refresh the	*/
		       /* list box					*/

	   plbw->cItems	+= LONGFROMMP(mp1);
	   RefreshList(plbw, 0L);
   ...
   case	LMX_SETARRAY :
	   ...
	   for ( i = 0;	i < LONGFROMMP(mp1); i++ )
	       if ( apsz[i] == NULL )
		   lSetItemText(plbw->plc[0].hHeap, hWnd, &plbw->plc[0].ali[i],	(PSZ)"") ;
	       else
		   if (	(lSetItemText(plbw->plc[0].hHeap, hWnd,	&plbw->plc[0].ali[i],
				      apsz[i]) > plbw->cxItem) && (plbw->flStyle & LS_HORZSCROLL) )
		       plbw->cxItem = plbw->plc[0].ali[i].cxItem;
   ...
   case	LMX_SETARRAYHANDLES :
	   ...
	   for ( i = 0;	i < LONGFROMMP(mp1); i++ )
	       plbw->plc[0].ali[i].ulHandle = aul[i];
   ...
   case	LMX_SETARRAYITEMS :
	   ...
	   albi	= (PLISTBOXITEM)PVOIDFROMMP(mp2);
	   ...
	   for ( i = 0;	i < LONGFROMMP(mp1); i++ )
	       ...
	       plbw->plc[0].ali[i].ulHandle = albi[i].ulHandle;
	       }
		       /* Save the list	box length and refresh the	*/
		       /* list box					*/
   ...
   case	LMX_SORT :
	   ...
	   case	LIT_SORTASCENDING :
		   ...
		   SortAscending(plbw->plc[0].ali, 0, plbw->cItems - 1);
	       break;
	   ...
	   case	LIT_SORTDESCENDING :
		   ...
		   SortDescending(plbw->plc[0].ali, 0, plbw->cItems - 1);
	       break;
   ...
   case	LMX_SELECTALL :
	   ...
	   {
	   for ( i = 0;	i < plbw->cItems; i++ )
	       if ( (plbw->plc[0].ali[i].fl & LI_SELECTED) == 0UL )
		   {
		   SaveSelectState(0L, i);
		   plbw->plc[0].ali[i].fl |= LI_SELECTED;
		   }
