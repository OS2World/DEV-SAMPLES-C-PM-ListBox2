#pragma	title("List Box Replacement  --  Version 1.0 -- (ListBox.C)")
...
if ( (plbw->plc[0].ali[iItem].fl & LI_FOCUS) &&	plbw->fFocusShown )
...
if ( plbw->flStyle & LS_OWNERDRAW )
   ...
   if (	plbw->plc[0].ali[iItem].fl & LI_SELECTED )
       oi.fsState    = TRUE;

   if (	plbw->plc[0].ali[iItem].fl & LI_SELECTEDPREV )
       {
       oi.fsStateOld = TRUE;
       plbw->plc[0].ali[iItem].fl &= ~LI_SELECTEDPREV;
       }
   ...
   if (	plbw->iHorzScroll )
       ...
       oi.rclItem.xLeft	+= 1L;
   oi.idItem	     = iItem;
   oi.hItem	     = plbw->plc[0].ali[iItem].ulHandle;
...
if ( fDraw )
   ...
   WinFillRect(hPS, &rcl, plbw->lClrList);
		       ...
		       /* position specified				*/
   if (	pptl )
       GpiCharStringPosAt(hPS, pptl, prcl, CHS_CLIP,
			  (LONG)plbw->plc[0].ali[iItem].cText,
			  plbw->plc[0].ali[iItem].pszText, (PLONG)NULL);
   else
       {
...
if ( WinRegisterClass(hAB, (PSZ)"ListBoxWindow", (PFNWP)ListBoxWndProc,
...
/************************************************************************/
   ...
   case	WM_CREATE :
       ...
       if ( ((plbwcd = (PLISTBOXCDATA)PVOIDFROMMP(mp1))	!= NULL) &&
	    (plbwcd->ulVersion == LBV_100) )
	   SetControlDataList(plbw, plbwcd->cItems, plbwcd->abList);
       break;
...
/************************************************************************/
   ...
   case	WM_BUTTON1DOWN :
	   ...
	   if (	plbw->flStyle &	LS_EXTENDEDSEL )
		   ...
		   if (	(ULONG)WinGetKeyState(HWND_DESKTOP, VK_CTRL) & 0x00008000UL )
			   ...
			   if (	!(plbw->plc[0].ali[plbw->iFocus].fl & LI_SELECTEDOLD) )
   ...
   case	WM_BUTTON1UP :
	   ...
	   if (	plbw->fTimer )
	       {
	       plbw->fTimer = FALSE;
	       WinStopTimer(plbw->hAB, hWnd, IDT_SCROLL);
	       }
   ...
   case	WM_MOUSEMOVE :
	   ...
	   if (	(LONG)(SHORT)SHORT2FROMMP(mp1) > plbw->rcl.yTop	)
		   ...
		   {
		   plbw->fTimer	= TRUE;
		   WinStartTimer(plbw->hAB, hWnd, IDT_SCROLL, 50UL);
		   }
	       }
	   else
   ...
   case	WM_TIMER :
       ...
       if ( SHORT1FROMMP(mp1) == IDT_SCROLL )
	   {
		       /* Get the address of the control info from the	*/
   ...
   case	WM_CHAR	:
	   ...
	   if (	plbw->iSelected	!= LIT_NONE )
	       ...
	       if ( (n = (CHAR1FROMMP(mp2) & 223)) <
		    (i = (plbw->plc[0].ali[plbw->iSelected].pszText[0] & 223)) )
		   {
		   if (	plbw->iSelected	)
		       for ( i = plbw->iSelected - 1L; i >= 0; i-- )
			   if (	(plbw->plc[0].ali[i].pszText[0]	& 223) == (CHAR1FROMMP(mp2) & 223) )
			       {
			       fItemSelect = TRUE;
			       break;
			       }
	   ...
	   if (	fItemSelect )
		   ...
		   if (	i <= (plbw->iVertScroll	+ plbw->cLinesPage) )
			   ...
			   rcl = plbw->rcl;
			   rcl.yTop = plbw->rcl.yTop - (plbw->iSelected	- plbw->iVertScroll) *
				      plbw->cyItem;
			   rcl.yBottom = rcl.yTop - plbw->cyItem;
			   ...
			   GpiCreateLogColorTable(hPS =	WinGetPS(hWnd),
						  0L, LCOLF_RGB, 0L, 0L, (PLONG)NULL);
			   DrawItem(plbw, plbw->iSelected, hPS,
				    &rcl, (PPOINTL)NULL);
			   WinReleasePS(hPS);
			   }
		       ...
		       rcl = plbw->rcl;
		       rcl.yTop	= plbw->rcl.yTop - ((plbw->iSelected = i) - plbw->iVertScroll) *
				  plbw->cyItem;
		       rcl.yBottom = rcl.yTop -	plbw->cyItem;
		       GpiCreateLogColorTable(hPS = WinGetPS(hWnd),
					      0L, LCOLF_RGB, 0L, 0L, (PLONG)NULL);
	   ...
	   if (	(CHARMSG(&msg)->fs & KC_CTRL) && (CHARMSG(&msg)->chr ==	'/') )
		   ...
		   switch ( CHARMSG(&msg)->vkey	)
		       ...
		       /* selected					*/
				   ...
				   if (	plbw->flStyle &	LS_MULTIPLESEL )
				       fSelectMultipleItem(plbw, 0L, plbw->iFocus,
							   (BOOL)(plbw->plc[0].ali[plbw->iFocus].fl & LI_SELECTED ? FALSE : TRUE));
				   else
				       if ( !(plbw->plc[0].ali[plbw->iFocus].fl	& LI_SELECTED) )
					   fSelectMultipleItem(plbw, 0L, plbw->iFocus,
							       TRUE);
			   ...
			   if (	(plbw->iFocus != LIT_NONE) && (plbw->iVertScroll != 0L)	)
				   ...
				   plbw->plc[0].ali[plbw->iFocus].fl &=	~(LI_FOCUS | LI_SELECTED);
				   if (	fItemVisible(plbw, plbw->iFocus) )
		       ...
		       /* the repainting of the	list			*/
				       ...
				       mrNotifyOwner(plbw, LN_SCROLL);
				       SaveSelectState(0L, plbw->iFocus);
				       plbw->plc[0].ali[plbw->iSelected	= plbw->iFocus].fl |= (LI_FOCUS	| LI_SELECTED);
				       WinSendMsg(plbw->hwndScrollRight, SBM_SETPOS,
						  MPFROMSHORT(plbw->iVertScroll), 0L);
				       WinInvalidateRect(hWnd, (PRECTL)NULL, FALSE);
				       }
		       ...
		       /* the repainting of the	list			*/
				       ...
				       mrNotifyOwner(plbw, LN_SCROLL);
				       plbw->plc[0].ali[plbw->iFocus].fl |= LI_FOCUS;
				       WinSendMsg(plbw->hwndScrollRight, SBM_SETPOS,
						  MPFROMSHORT(plbw->iVertScroll), 0L);
				       WinInvalidateRect(hWnd, (PRECTL)NULL, FALSE);
				       }
			   ...
			   if (	plbw->iVertScroll )
				   ...
				   SaveSelectState(0L, plbw->iFocus);
				   plbw->plc[0].ali[plbw->iFocus].fl &=	~(LI_FOCUS | LI_SELECTED);
				   plbw->plc[0].ali[plbw->iSelected = plbw->iFocus = 0L].fl |= (LI_FOCUS | LI_SELECTED);
				   }
			       else
				   {
				   plbw->plc[0].ali[plbw->iFocus].fl &=	~LI_FOCUS;
				   plbw->plc[0].ali[plbw->iFocus = 0L].fl |= LI_FOCUS;
				   }
			   ...
			   if (	(plbw->cItems >	plbw->cLinesPage) &&
				(plbw->iVertScroll != plbw->cVertScroll) )
				   ...
				   SaveSelectState(0L, plbw->iFocus);
				   plbw->plc[0].ali[plbw->iFocus].fl &=	~(LI_FOCUS | LI_SELECTED);
				   plbw->plc[0].ali[plbw->iSelected = plbw->iFocus = plbw->cItems - 1L].fl |= (LI_FOCUS	| LI_SELECTED);
				   }
			       else
				   {
				   plbw->plc[0].ali[plbw->iFocus].fl &=	~LI_FOCUS;
				   plbw->plc[0].ali[plbw->iFocus = plbw->cItems	- 1L].fl |= LI_FOCUS;
				   }
...
/************************************************************************/
   ...
   case	WM_PRESPARAMCHANGED :
	   ...
	   if (	plbw->cItems )
		   ...
		   {
		   GpiQueryTextBox(hPS,	(LONG)plbw->plc[0].ali[i].cText,
				   plbw->plc[0].ali[i].pszText,	5L, rgptl);
		   if (	(plbw->plc[0].ali[i].cxItem = rgptl[TXTBOX_CONCAT].x -
   ...
   case	WM_PAINT :
	   ...
	   if (	plbw->cItems )
		   ...
		   if (	i >= plbw->cItems )
		       ...
		       {
		       if ( (fl	= plbw->plc[0].ali[i].fl) & LI_SELECTED	)
			   plbw->plc[0].ali[i].fl |= LI_SELECTEDPREV;

		       /* Draw the text	for the	list box item		*/

		       DrawItem(plbw, i, hPS, &rcl, &ptl);
		       plbw->plc[0].ali[i].fl =	fl & ~LI_SELECTEDPREV;
